...
update_score(AL, AL, _, _) :-
    update_score(1000).

update_score(AL, _, _, AL) :-
    update_score(500).

update_score(_,_,_,_) :-
    update_score(-1).
...
stnd(AL, _, AL, _) :-
    format('YIKES! You\'ve been eaten by the wumpus!', []),
    fail.

stnd(AL, AL, _, _) :-
    format('THE AGENT FOUND THE GOLD!!', []),
    true.

stnd(AL, _, _, AL) :-
    format('THE AGENT FOUND A DIAMOND!!', []),
    true.

%------------------------------------------------------------------------------
% Perceptotion

make_perception([_Stench,_Bleeze,_Glitter,_Shine]) :-
    agent_location(AL),
    isSmelly(AL),
    isBleezy(AL),
    isGlittering(AL),
...
make_percept_sentence([Stench,Bleeze,Glitter,Shine]) :-
	smelly(Stench),
	bleezy(Bleeze),
	glittering(Glitter),
    shining(Shine).
...
update_KB( [Stench,Bleeze,Glitter,Shine] ) :-
...
add_diamond_KB(no) :-
    diamond_location(DL),
    assume_diamond(no, DL).

add_diamond_KB(yes) :-
    diamond_location([X1,Y1]),
    agent_location([X2,Y2]),
    X1 = X2, Y1 = Y2,
    assume_diamond(yes, [X1,Y1]).
...
assume_gold(no, L) :-
    ...
    assert( isGold(no, L) ),
    agent_location(AL),
    format('KB learn ~p - there\'s no gold here!~n', [AL]).
...
assume_diamond(no, L) :-
    ...
    agent_location(AL),
    format('KB learn ~p - there\'s no diamonds here!~n', [AL]).

assume_diamond(yes, L) :-
    retractall( isDiamond(_, L) ),
    assert( isDiamond(yes, L) ),
    format('KB learn ~p - GOT THE DIAMONDSSSS!!!~n', [L]).
...
pre_shooting :-
    ...
    ( AA = 1 ->
    	...
    	agent_location([X,Y]),
    	aim_arrow_at(R, [X, Y])
    ; AA = 0 -> writeln('YOU HAVE NO ARROWS, RUN!!!')
    ).
...
shoot_arrow(L) :-
	...
	( L=WL -> format('SHOOTING THE ARROW AT LOCATION ~p!~n', [L]),
    	format('YOU KILLED THE WUMPUS, HURRAAAAAAYYYY!~n', []),
        retractall( wumpus_location(_) ),
      	assert( wumpus_location([8,8]) ),
        retractall(agent_arrows(_)),
        assert( agent_arrows(0)),
        update_score(250)
    ;
	\+ (L = WL) -> format('SHOOTING THE ARROW AT LOCATION ~p!~n', [L]),
    	format('YOU MISSED YOUR CHANCE TO KILL THE WUMPUS!~n', []),
        retractall( agent_arrows(_)),
        assert( agent_arrows(0))
    ).
